...
//   const [dbUserId, setDbUserId] = useState<string | null>(null);
//   const [subscriptionStatus, setSubscriptionStatus] = useState<boolean>(true); // Defaulted to true as in original code
//   const [subscriptionPlan, setSubscriptionPlan] = useState<string>("");

//   // Roadmap & Form Status State
//   const [hasRoadmap, setHasRoadmap] = useState<boolean>(false);
//   const [formFilled, setFormFilled] = useState<boolean | null>(null);
//   const [apiCallCompleted, setApiCallCompleted] = useState(false);
...
//   // Fetch user data, subscription, and career info
//   useEffect(() => {
//     if (!isLoaded || !user) return;
//     let isMounted = true;
...
//         const roadmapExists = !!(careerData?.roadmap && Object.keys(careerData.roadmap).length > 0);
//         setHasRoadmap(roadmapExists);
//         setFormFilled(careerData?.form_filled ?? false);

//         if (careerData) {
//             const findOpt = (val?: string) => {
//                 // Assuming countryOptions is available; it's better to define it once at the top level
//                 const countryOptions = (countryList().getData() as OptionType[]);
//                 return countryOptions.find((o) => o.value === val) || null;
//             }
...
//             setWillingToMoveAbroad(careerData.move_abroad ?? false);
//             setMoveAbroad(careerData.move_abroad ? "yes" : "suggest");
//             setPreferredAbroadCountry(findOpt(careerData.preferred_abroad_country));
//             setIsCollegeStudent(careerData.college_student ?? null);
//             setDifficulty((careerData.difficulty as "easy" | "medium" | "hard") ?? null);
...
//   // Fetch universities when college form is opened
//   useEffect(() => {
//     if (showCollegeForm) {
//       const fetchUniversities = async () => {
//         try {
...
//   // Redirect after roadmap generation
//   useEffect(() => {
//     if (apiCallCompleted && (!isCollegeStudent || formFilled || (isCollegeStudent && residingCountry?.value !== 'IN'))) {
//         console.log("Conditions met; redirecting to /roadmap.");
//         setGenerating(false);
//         router.push("/roadmap");
//     }
//   }, [apiCallCompleted, formFilled, isCollegeStudent, residingCountry, router]);
...
//       preferred_abroad_country: willingToMoveAbroad && moveAbroad === "yes" ? preferredAbroadCountry?.value : (willingToMoveAbroad ? "Suggest" : null),
//       parent_email: parentEmail,
//       college_student: isCollegeStudent,
//       difficulty: difficulty,
//       roadmap: null,
...
//     try {
//       await fetch("/api/save-career-info", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
...
//   const handleGenerateRoadmap = async () => {
//     setShowGenerateModal(false); // Close the confirmation modal first
//     setGenerating(true);

//     // If Indian college student needs to fill the form, show it. The actual generation will be triggered after that form is submitted.
...
//     try {
//       await fetch("/api/generate-roadmap", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ clerk_id: user?.id }) });
//       setApiCallCompleted(true);
//     } catch (error) {
//       console.log("Error generating roadmap:", error);
//       setGenerating(false);
...
//   const handleCollegeFormSubmit = async (e: React.FormEvent) => {
//     e.preventDefault();
//     if (!dbUserId || !selectedUniversityId) {
//       alert("Please select a university.");
//       return;
//     }
...
//       // Now trigger the roadmap generation that was paused.
...
//         {hasRoadmap ? <RoadmapNotification /> : (
//           <div className="mb-6 p-3 bg-orange-50 text-orange-700 rounded-md">
//             <p>Fill in the fields below to get your personalized career roadmap.</p>
//           </div>
//         )}
...
//             <div className="flex justify-end space-x-4">
//               <button onClick={() => setShowGenerateModal(false)} className="bg-white text-black py-5 px-8 rounded-full border-2 border-black hover:border-transparent transition-all duration-500 hover:bg-red-500 mb-2">Cancel</button>
//               <button onClick={handleGenerateRoadmap} className="bg-white text-black py-5 px-8 rounded-full border-2 border-black hover:border-transparent transition-all duration-500 hover:bg-green-400 mb-2">Generate</button>
//             </div>
//           </div>
//         </div>
//       )}
...
//         setInputValue={setUniversityInputValue}
