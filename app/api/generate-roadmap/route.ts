...
//     },
//     body: JSON.stringify({
//       model: "google/gemini-2.0-flash-exp:free", // You can adjust the model as needed
//       messages: [
//         {
//           role: "user",
//           content: `You are Careeroadmap's AI expert. Always respond strictly in JSON format with no markdown or extra text. For each task:
// - If the description mentions a YouTube videos, include either a "video" object { title, url, thumbnail } or a "video_channel" object { name, url }, try not to keep same names of the items inside the object, it causes error.
// - Detect any mentioned platforms (e.g., Coursera, Kaggle, HackerRank, or any other) and include a "platform_links" array of { name, url }.
// - Provide an "importance_explanation" field: 1–2 sentences explaining why the task's weight matters.
// - Keep existing fields: task_title, description, weight, completed.
// Ensure the final JSON matches the structure: { career, roadmap_summary, yearly_roadmap, final_notes }.
...
//   // Extract the generated text from OpenRouter response format
//   if (!data.choices || data.choices.length === 0) {
//     console.error("Empty response from OpenRouter API:", data);
...
//   // Strip any ```json fences or capture the JSON object
//   const match = raw.match(/```json\s*([\s\S]*?)\s*```/i) || raw.match(/({[\s\S]*})/);
//   if (!match) {
//     console.error("No JSON payload found in OpenRouter reply:", raw);
//     throw new Error("Failed to locate JSON in OpenRouter response");
//   }
...
// - Must Include the YouTube tutorials/videos or websites in the tasks if required that might be helpful. 
// The response must be strictly in JSON format without any additional text, markdown formatting, or backticks, and must exactly match the following structure:
...
//       "phases": [
//         {
//           "phase_name": "Phase 1 (Month 1 with year - Month 3 with year)",
//           "milestones": [
//             {
//               "name": "Milestone Name",
//               "description": "What needs to be achieved in this milestone",
