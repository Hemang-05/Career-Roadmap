...
export async function GET(request: Request) {
  ...
  try {
    ...
    console.log(`Found ${subscribedUsers?.length || 0} users with active subscriptions`);
      ...
      try {
        ...
        const needsUpdate = 
          ...
          (new Date(mostRecentEvent.created_at).getTime() < 
           new Date(currentDate.getTime() - 15 * 24 * 60 * 60 * 1000).getTime());
    ...
    console.log(`Processing events for ${usersNeedingUpdates.length} subscribed users who need updates`);
    ...
    for (const userInfo of usersNeedingUpdates) {
      try {
        ...
        if (insertError) {
          console.error(`Error storing events for user ${userInfo.user_id}:`, insertError);
          continue;
        }
        ...
        console.log(`Successfully processed events for user ${userInfo.user_id}`);
      } catch (err: any) {
