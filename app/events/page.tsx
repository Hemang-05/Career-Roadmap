...
export default function EventsPage() {
  ...
  const handleExternalSearch = async () => {
    if (!desiredCareer) {
      setExternalError('Desired career is not available. Please update your career info.');
      return;
    }
    if (!user?.id) {
  ...
  useEffect(() => {
    if (isLoaded && user && desiredCareer) {
      ...
      if ((today === 18 || today === 15) && triggeredMonth !== currentMonth) {
        console.log('Auto-triggering external events search for current month:', currentMonth);
  ...
  useEffect(() => {
    const handleClickOutsideCard = (event: MouseEvent) => {
      ...
      if (isHorizontalCardVisible && card && !card.contains(event.target as Node)) {
        closeHorizontalCard();
      }
    };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <div className="container mx-auto px-4 py-8 flex-grow mt-24">
        <h1 className="text-3xl font-bold mb-8 text-gray-800"><span className='text-[#FF6500]'>Career</span> Events & Opportunities</h1>

        {/* Fancy Month Dropdown */}
        <div className="mb-8 relative" id="month-dropdown">
          <label className="block text-gray-700 mb-2 font-medium">Select Month:</label>
          <div 
          ...
          >
            <div className="flex items-center justify-between p-3 bg-white border border-gray-300 rounded-md shadow-sm hover:border-[#FF6500] transition-all duration-200">
              <span className="text-gray-800">{selectedMonth || "-- Select Month --"}</span>
              <svg 
            ...
            {isDropdownOpen && (
              <div className="absolute mt-1 w-full bg-white border border-gray-200 rounded-md shadow-lg z-10 max-h-60 overflow-auto py-1 transform transition-all duration-200 ease-in-out origin-top">
                <div className="py-1 px-2 text-xs text-gray-500 border-b border-gray-100">Select a month</div>
                {months.map((month) => (
                  <div
                    key={month}
                    className={`px-4 py-2 hover:bg-gray-50 cursor-pointer transition-colors duration-150 ${
                      selectedMonth === month ? 'bg-orange-50 text-[#FF6500]' : 'text-gray-700'
                    }`}
                    onClick={() => {
        ...
        {loadingStored ? (
          ...
          storedEvents.map((row) => (
            <div key={row.id} className="mb-8">
              <h2 className="text-2xl font-bold mb-6 text-gray-800">Events for {row.event_month}</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {Array.isArray(row.event_json) && row.event_json.length > 0 ? (
                  row.event_json.map((event: any, idx: number) => (
                    ...
                    >
                      <div className="w-64 h-80 bg-white shadow-[0px_0px_15px_rgba(0,0,0,0.09)] p-6 space-y-3 relative overflow-hidden transition-transform duration-300 hover:scale-105">
                          ...
                          <div className="w-24 h-24">
                            {idx === 0 && (
                              <img src="/event2.webp" alt="Triangle" className="w-full h-full object-contain" />
                            )}
                            {idx === 1 && (
                              <img src="/event5.webp" alt="Line" className="w-full h-full object-contain" />
                            )}
                            {idx === 2 && (
                              <img src="/event1.webp" alt="Circle" className="w-full h-full object-contain" />
                            )}
                            {idx === 3 && (
                              <img src="/event4.webp" alt="Circle" className="w-full h-full object-contain" />
                            )}
                            {idx === 4 && (
                              <img src="/event3.webp" alt="Circle" className="w-full h-full object-contain" />
                            )}
                          </div>

                          <h1 className="font-bold text-gray-700 text-lg line-clamp-3">{event.title}</h1>
                        
                        </div>
                    </div>
                  ))
                ) : (
                  <p className="text-gray-600 col-span-3">No events stored in JSON for this month.</p>
                )}
              </div>
            </div>
          ))
          ...
          <div className="bg-gray-50 rounded-lg p-8 text-center">
            ...
            <p className="text-gray-600 mb-4">No events found for {selectedMonth || "the selected month"}.</p>
            <p className="text-sm text-gray-500">Select a different month from the dropdown or check back later for updates.</p>
          </div>
        )}
        ...
        {selectedEvent && (
          <div 
            className={`fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 transition-opacity duration-300 ${
              isHorizontalCardVisible ? 'opacity-100' : 'opacity-0 pointer-events-none'
            }`}
          >
            <div 
            ...
            >
              <div className="flex flex-col md:flex-row">
                ...
                <div className="md:w-1/3 bg-[#FF6500] p-6 flex items-center justify-center">
                ...
                <div className="md:w-2/3 p-6 md:p-8">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    >
                      <svg className=" absolute w-6 h-6 top-2 right-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <div className="mb-6 h-64 overflow-y-auto pr-2">
                    <p className="text-gray-600 ">
                      {selectedEvent.content || "This is a career event that matches your interests. Click the Check-In button below to learn more and register for this opportunity."}
                    </p>
                  </div>
      ...
      <footer className="bg-gray-50 border-t mt-auto">
        <div className="container mx-auto px-4 py-4 text-center text-gray-500 text-sm">
          &copy; {new Date().getFullYear()} CareerRoadmap. All rights reserved.
        </div>
      </footer>
    </div>
  );
