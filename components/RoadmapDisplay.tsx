  ...
  return (
    <div className="space-y-6 md:space-y-8">
      {roadmapData.yearly_roadmap.map((yearItem: any, yearIndex: number) => {
        ...
        const unlocked = isFirstYear || (prevYear ? isYearComplete(prevYear) : true);
              ...
              (yearItem.phases || []).map((phaseItem: any, pIdx: number) => (
                ...
                >
                  ...
                  {phaseItem.description && (
                    <p className="text-gray-600 text-sm md:text-base mb-4">{phaseItem.description}</p>
                  )}
                  
                  {/* Show milestones */}
                  {(phaseItem.milestones || []).map((milestone: any, mIdx: number) => (
                    <div key={milestone.name || mIdx} className="mx-0 md:mx-2 lg:mx-4 mb-4 md:mb-6">
                          ...
                          >
                            ...
                            <div
                              onClick={() => {
                                ...
                                const newRoadmapData = JSON.parse(JSON.stringify(roadmapData)); // Deep copy
                                const currentTask = newRoadmapData.yearly_roadmap[yearIndex].phases[pIdx].milestones[mIdx].tasks[tIdx];
                                ...
                                fetch('/api/update-task', {
                                  ...
                                  body: JSON.stringify({
                                    ...
                                    currentPhaseIdentifier: phaseItem.phase_name, 
                                  }),
                                })
                                .then(async (res) => {
                                  if (!res.ok) {
                                    // Revert on failure
                                    const revertedRoadmap = JSON.parse(JSON.stringify(roadmapData)); // Get original copy
                                ...
                                .catch(() => {
                                  ...
                                  const revertedRoadmap = JSON.parse(JSON.stringify(roadmapData));
                                  onTaskUpdate(revertedRoadmap);
                                });
                              }}
                              className={`
                            ...
                            <div className="flex-grow">
                              <div className="flex justify-between items-start flex-wrap">
                                ...
                                <div className="flex items-center ml-auto">
                                  {task.importance_explanation && (
                                    <Tooltip label={task.importance_explanation}>
                              ...
                              <div className="mt-2">
                                ...
                                {task.youtube_url && <YouTubeEmbed url={task.youtube_url} title={task.task_title} />}
                                
                                {/* Original video handling */}
                                {task.video?.url && <YouTubeEmbed url={task.video.url} title={task.video.title || task.task_title} />}

                                {/* Video channel link with updated styling */}
                                {task.video_channel?.url && !getYouTubeEmbedUrl(task.video_channel.url) && (
                                ...
                                {/* External links */}
                                {(task.platform_links || []).map((link: any, i: number) => (
                                  <a
                                    key={link.url || i}
                                    href={link.url}
                                    target="_blank"
                  ...
                  <>
                    ...
                    {unlocked && (
                      <div className="border-t pt-4 mt-4 text-center">
                        <p className="text-gray-600 text-sm">Click to view tasks for this year.</p>
                      </div>
                    )}
                  </>
                )}
      ...
      {roadmapData.final_notes && (
        <div className="mt-6 md:mt-8 p-4 border-t">
          <h3 className="text-xl font-semibold text-gray-800 mb-2">Final Notes</h3>
